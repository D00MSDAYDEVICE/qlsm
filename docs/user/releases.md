...
| --- | --- | --- | --- |
| `v1.32.0` | 2026-09-12 | [#209](https://github.com/dngrtech/qlsm/pull/209) | Make Redis the only source of truth for minqlx admin levels. The **Owner & Admins** tab shows the admins the server has right now (including in-game `!setperm` changes) and loads as soon as Edit Configuration opens, saving writes only the admins you changed, presets include the server's admin list, **Add to operators** opens the Add Operator dialog in place, and numeric `steamid|level` lines are no longer written into `access.txt`. |
