...
| --- | --- | --- | --- |
| `v1.37.1` | 2026-09-16 | [#217](https://github.com/dngrtech/qlsm/pull/217) | Fix plugins downloaded from a plugin repository never reaching your hosts on Docker installs. Downloads now go to `data/shared-plugins/<runtime>/`, which every part of QLSM can see and which survives updates. Plugins downloaded before this version were stored inside the web container only and must be downloaded again. Backups now include downloaded plugins instead of QLSM's built-in ones. |
