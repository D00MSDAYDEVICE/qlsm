...
1. Open the instance's config (or **Deploy A New Instance**) and go to the **Plugins** tab. The plugin appears as a **shared** row (see [Shared Plugins](edit-configs.md#shared-plugins)).
2. If the row also shows **not on host**, the host has not received the file yet. Click the upload icon next to it to push the shared folder to that host, and wait for the badge to clear.
